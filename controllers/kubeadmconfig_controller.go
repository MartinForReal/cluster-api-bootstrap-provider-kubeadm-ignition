...
import (
	...
	"github.com/minsheng-fintech-corp-ltd/cluster-api-bootstrap-provider-kubeadm-ignition/internal/locking"
...
type KubeadmConfigReconciler struct {
	...
	IgnitionFactory    *ignition.Factory
}
...
func (r *KubeadmConfigReconciler) handleClusterNotInitialized(ctx context.Context, scope *Scope) (_ ctrl.Result, reterr error) {
	...
	cloudInitData, err := r.IgnitionFactory.GenerateUserData(&ignition.Node{
		...
		Services: []types.ServiceUnit{
			{
				...
				Content: fmt.Sprintf(types.InitUnitTemplate, verbosityFlag, types.KubeadmConfigPath),
				Enabled: true,
